...
static uint8_t new_period = 0;                                 /**< A new period measurement has been started */
...
static float Kp = 0.5f;  // Пропорциональный коэффициент
static float Ki = 0.01f; /**< The integral coefficient, TODO: test with Ki=0.0003; */
static float Kd = 0.1f; // Дифференциальный коэффициент
 ...
 *
 * @param et The current value of the error
...
static void adc_half_cplt_callback(void)
{
    ...
    else
    {
        ...
        // Идеальные синусоиды для каждой фазы
        float idealA = sin(symbol / 128.0f * 2.0f * M_PI);
        float idealB = sin(symbol / 128.0f * 2.0f * M_PI + 2.0f * M_PI / 3.0f);
        float idealC = sin(symbol / 128.0f * 2.0f * M_PI + 4.0f * M_PI / 3.0f);

        // Реальные измерения
        float realA = pfc.adc.ch[last_buffer][ADC_MATH_A][symbol] / pfc.adc.active[ADC_MATH_A];
        float realB = pfc.adc.ch[last_buffer][ADC_MATH_B][symbol] / pfc.adc.active[ADC_MATH_B];
        float realC = pfc.adc.ch[last_buffer][ADC_MATH_C][symbol] / pfc.adc.active[ADC_MATH_C];

        // Ошибки
        float errorA = realA - idealA;
        float errorB = realB - idealB;
        float errorC = realC - idealC;

        // Применяем ПИД-регуляторы
        float va = PID(errorA, &Ia_e_1, 0, Ki, 0, &Ia_It_1);
        float vb = PID(errorB, &Ib_e_1, 0, Ki, 0, &Ib_It_1);
        float vc = PID(errorC, &Ic_e_1, 0, Ki, 0, &Ic_It_1);
